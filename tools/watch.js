...
var funcUtils = require('./func-utils.js');
...
_.extend(Watcher.prototype, {
  ...
  _watchFileOrDirectory: function (absPath) {
    ...
    function onWatchEvent() {
      if (! lazyCallback) {
        // When pathwatcher.watch throws an exception because the file
        // does not (yet) exist, we don't want to go to the trouble of
        // actually creating the callback.
        lazyCallback = self._makeWatchEventCallback(absPath);
      }
      lazyCallback();
    }
  ...
  _makeWatchEventCallback: function (absPath) {
    ...
    return funcUtils.coalesce(100, function onWatchEvent() {
      if (self.stopped) {
        return;
      }
